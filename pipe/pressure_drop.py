...
def build_ui():
    ...
    if st.session_state['calc_run']:
        ...
        tab1, tab2, tab3 = st.tabs(["📊 기본 시뮬레이션 결과", "⚖️ 단일 조건 비교 분석", "💸 경제적 최적 관경(Economic Diameter)"])
        ...
        with tab3:
            ...
            **생애주기비용 (LCC: Life Cycle Cost) 기반 최적화 논리**: 
            ...
            with res_col1:
                ...
                if eco_auto_pump:
                    st.caption(f"↳ 자동 산출된 펌프 용량: **{opt_details['kw']:.1f} kW** (추정 효율 **{opt_details['eff']:.1f}%**, 펌프 가격: **{opt_details['pump_cost']:,.0f}원**)")
